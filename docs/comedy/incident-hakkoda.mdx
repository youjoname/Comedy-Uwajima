...
日:ジョー:その通りだ。それでは、少し休憩を取ろう。戻ってきたら、宇宙旅行の未来について掘り下げるよ。ついにジェットパックが手に入るのか？　お楽しみに。
  ...
  - 口語表現（Colloquialisms）
      - getting their asses kicked: やられた
      - Holy shit: なんてこった
  - 慣用句（Idioms）
      - think ahead: 先を見越す
      - fell apart: 崩壊した
      - the whole nine yards: すべてが最悪
  - スラング（Slang）
      - crap: くだらないもの
  - 比喩（Metaphors）
      - giant, angry snow globe: 巨大で怒ったスノーグローブ
  - 句動詞（Phrasal Verbs）
      - hit this insane snowstorm: とんでもない吹雪に遭遇した
  - 専門用語（Jargon）
      - brass: 軍の高官
  - 決まり文句（Clichés）
      - Nature always wins: 自然はいつも勝つ
  - ポップカルチャーからの引用（Pop culture references）
      - Donner Party: ドナー隊（アメリカの開拓時代の悲劇的な出来事）
  </TabItem>
</Tabs>