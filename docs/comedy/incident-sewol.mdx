...
日:ゲスト：いつでもどうぞ、ジョー。漫才でも安全でも、すべてはバランスが大切なんです。バランスを失えば、夕方のニュースの笑い話になってしまうかもしれません。
  ...
  - 慣用句（Idioms）
      - break it down: 説明する
      - play submarine: 潜水艦ごっこをする
      - everything but the kitchen sink: ほとんどすべてのもの
  - スラング（Slang）
      - ditched: 逃げ出した
  - 比喩（Metaphors）
      - floating disaster waiting to happen: 起こるのを待っていた浮かんだ災難
  - 句動詞（Phrasal Verbs）
      - diving into: 掘り下げる
      - stay put: そのままいる
  - 決まり文句（Clichés）
      - wake-up call: 目覚めの呼び声
  - ポップカルチャーからの引用（Pop culture references）
      - Fast and Furious: ファストアンドフューリアス（映画シリーズ）
  </TabItem>
</Tabs>