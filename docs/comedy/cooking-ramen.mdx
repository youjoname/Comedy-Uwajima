...
日: ゲスト: 忘れるなよ、ジョー。未来ではすべての食事がラーメンだ。朝食？　エッグベネディクトラーメン。昼食？　ラーメンブリトー。夕食？　ラーメンラザニア。これは単なる食べ物じゃない。スイスアーミーナイフ、ライトセーバー、そしてバイブレーターを1つの湯気立つズルズルパッケージに詰め込んだような料理の化身なんだ。未来はもうここにあるぜ、そしてそれは超絶品のラーメンの味がするんだ！
  ...
  <TabItem value="word list" label="単語">
    - 口語表現（Colloquialisms）
        - Holy shit: なんてこった
        - You're goddamn right: その通りだ、くそったれ
        - Abso-fucking-lutely: 絶対に、くそったれ
    - 慣用句（Idioms）
        - file a restraining order: 接近禁止令を出す
        - make obsolete: 時代遅れにする
    - スラング（Slang）
        - Bam: バン（擬音語）
        - weak-ass: 弱っちい
    - 比喩（Metaphors）
        - so bright, you'll need sunglasses: 明るすぎて、サングラスが必要
        - steaming bowl of heaven: 湯気立つ天国のラーメン
        - noodle-based economy: 麺ベースの経済
    - 句動詞（Phrasal Verbs）
        - go obsolete: 時代遅れになる
        - get this: これを聞け
    - 決まり文句（Clichés）
        - You heard it here first: ここで初めて聞いた
    - ポップカルチャーからの引用（Pop culture references）
        - Bitcoin: ビットコイン
        - 401k: 401kプラン（アメリカの確定拠出年金制度）
        - X Games: エックスゲーム（極限スポーツの大会）
        - MMA: 総合格闘技
